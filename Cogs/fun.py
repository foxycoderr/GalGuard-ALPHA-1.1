...
# [GIFS]
  ...
  async def feed(self, ctx, member: discord.Member):
    joke = discord.Embed(title="Feeding time!", description=f"{ctx.message.author} fed {member}!", color=0x487799, timestamp = ctx.message.created_at)
  ...
  async def kill(self, ctx, member: discord.Member):
    joke = discord.Embed(title="One shot. One kill.", description=f"{ctx.message.author} just murdered {member}!", color=0x487799, timestamp = ctx.message.created_at)
...
# [DICE]
  ...
  async def dice(self, ctx, number=1):
      result = discord.Embed(title="🎲 Dice roller", description="🪄 Rolling your dice...", color=0x487799, timestamp = ctx.message.created_at)
